...
model_bucket_name = os.enviorn('model_bucket_name')

# Defining a function that Imports the dataset
def read_csv_from_s3(s3_url):
	bucketname = s3_url.split('/')[2].split('.')[0]
	file_key = '/'.join(s3.url.split('/')[3:])
	s3 = boto3.client('s3') # interact with S3 client
	csv_obj = s3.get_object(Bucket=bucket_name,Key=file_key)
	csv_string = csv_obj['Body'].read().decode('utf-8')
	csv_reader = csv.reader(StringIO(csv_string), delimitere=';')
...
def split_data(data, test_ratio=0.10):
	...
	return data[:split_index],data[split_index:] # return training data and test data
...
def train_evaluate_save(event, context):
    ...
    #serialization of regressor in bytes format
    temp_file_path = '/tmp/' + model_name
  	with open(temp_file_path, 'wb') as f1: # wb - write binary
   		pickle.dump(regressor, f1)
   	
   	with open(temp_file_path, 'rb') as f2: #rb - read binary
   		model_data = f2.read()
...
def predict_wine_quality(event, context):
    ...
    with open(temp_file_path, 'rb') as f:
        model = pickle.load(f)
    return str(round(model.predict([input_data])[0], 1)) # input is in 2-d array format